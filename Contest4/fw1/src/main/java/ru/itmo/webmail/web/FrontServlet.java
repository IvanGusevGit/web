...
public class FrontServlet extends HttpServlet {
    ...
    private void processRoute(Route route, HttpServletRequest request, HttpServletResponse response) throws NotFoundException, ServletException, IOException {
        ...
        Method method = makeMethod(clazz, route.getAction());
        if (method == null) {
        ...
        makeAndInvokeMethod(request, response, view, clazz, page, "before");
        makeAndInvokeMethod(request, response, view, clazz, page, route.getAction());
        makeAndInvokeMethod(request, response, view, clazz, page, "after");
    ...
    private void makeAndInvokeMethod(HttpServletRequest request, HttpServletResponse response,  Map<String, Object> view, Class<?> clazz, Object page, String methodName) throws IOException, ServletException{
        Method method = makeMethod(clazz, methodName);
        try {
            invokeAction(page, request, view, method);
    ...
    private Method makeMethod(Class<?> clazz, String methodName) {
        for (Class<?> c = clazz; c != null; c = c.getSuperclass()) {
            Optional<Method> candidate = Arrays.stream(c.getDeclaredMethods()).filter(method -> method.getName().equals(methodName)).findFirst();
